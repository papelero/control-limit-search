...
class SplitPointsSearch:
    ...
    def __filter_split_points(self, array, labels, precision_limits, *args):
        ...
        if args:
            if args < threshold:
                mask = threshold < splits
            else:
                mask = splits < threshold
        else:
            if threshold < self.median_ok:
                mask = splits < threshold
            else:
                mask = threshold < splits
        return splits[mask]

    def optimal_split_point(self, array, labels, precision_limits, *args):
        ...
        if args:
            split_points = self.__filter_split_points(array, labels, precision_limits, args)
        else:
        ...
        for idx, split in enumerate(split_points):
            ...
            for label in np.unique(labels):
                class_count_lower.append(array[np.logical_and(labels == label, array < split)].size)
                class_count_upper.append(array[np.logical_and(labels == label, split < array)].size)
    ...
    def fit(self):
        ...
        split2 = self.optimal_split_point(self.array[:, self.time_step], self.labels, self.inverse_precision, split1)
        if split1 < split2:
